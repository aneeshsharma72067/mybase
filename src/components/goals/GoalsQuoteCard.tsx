...
const fallbackQuote = 'Nature does not hurry, yet everything is accomplished.'
const fallbackAuthor = '- Lao Tzu'
...
export function GoalsQuoteCard() {
  ...
  const quote = pinnedQuote?.quoteText?.trim() || fallbackQuote
  ...
  return (
    <article className="flex h-80 flex-col items-center justify-center rounded-xl bg-secondary p-8 text-center text-on-secondary lg:h-96">
      ...
      <p className="mt-5 text-sm italic opacity-75">{pinnedQuote ? author : fallbackAuthor}</p>
    </article>
  )
}