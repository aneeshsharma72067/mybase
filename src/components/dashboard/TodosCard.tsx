    ...
    <section className="rounded-xl bg-surface-container-lowest p-6">
      ...
      <div className="mt-5 flex items-center gap-2">
        <input
          ...
          className="w-full rounded-full border border-outline-variant bg-surface px-4 py-2 text-sm text-on-surface outline-none transition-colors focus:border-primary"
        />
        <button
          type="button"
          onClick={handleAddTodo}
          disabled={!nextTodo.trim()}
          aria-label="Add task"
          className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-primary text-on-primary transition-transform active:scale-95 disabled:opacity-40"
        >
          <Plus size={16} />
        </button>
      </div>
